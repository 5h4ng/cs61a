...
class SmartFridge:
    ...
    def use_item(self, item, quantity):
        "*** YOUR CODE HERE ***"
        tmp = self.items[item] - quantity
        if tmp <= 0:
            self.items[item] = 0
            return "Oh no, we need more " + item + "!"
        else:
            self.items[item] = tmp
            return "I have " + str(tmp) + " " + item + " left"
    ...
    def play_round(self, starter, cards):
        ...
        for who, card in cards:
            try:
                ...
                print(e)
        if r.winner != None and r.is_complete():
...
class Round:
    ...
    def play(self, who, card):
        ...
        if card >= self.highest:
            ...
            self.winner = who
        if self.is_complete():
            self.next_player = self.starter

    def is_complete(self):
        """ Checks if a game could end. """
        return self.next_player == self.starter and self.winner != None
