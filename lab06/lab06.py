...
def repeated(t, k):
    ...
    assert k > 1
    "*** YOUR CODE HERE ***"
    def helper(t, k, last, count):
        if count == k:
            return last
        else:
            current = next(t)
            if current == last:
                return helper(t, k, current, count + 1)
            else:
                return helper(t, k, current, 1)
    return helper(t, k, next(t), 1)
...
def pizza_sort_helper(lst, start):
    if start != len(lst):
        #print(lst, start, index_largest(lst[start:]))
        partial_reverse(lst, index_largest(lst[start:]) + start)
        #print(lst, start, lst[start:])
