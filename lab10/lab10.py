...
def reverse_other(t):
    ...
    """
    "*** YOUR CODE HERE ***"
    def reverse_helper(t, odd_depth):
        if (not t.is_leaf()) and (odd_depth):
            labels = [b.label for b in t.branches][::-1]
            for branch, label in zip(t.branches, labels):
                branch.label = label
        for branch in t.branches:
            reverse_helper(branch, not odd_depth)
