...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    if typed_word in word_list:
        ...
        for i in range(len(word_list)):
            if diff_function(typed_word, word_list[i], limit) < diff_function(typed_word, word_list[min_diff_index], limit):
                min_diff_index = i
        if diff_function(typed_word, word_list[min_diff_index], limit) > limit:
            return typed_word
        else:
...
def fastest_words(match):
    ...
    "*** YOUR CODE HERE ***"
    fastest = []
    is_chosen = [0 for i in word_indices]
    for i in player_indices:
        tmp = []
        for j in  word_indices:
            if time(match, i, j) == min([time(match, k, j) for k in player_indices]) and is_chosen[j] == 0:
                tmp += [get_word(match, j)]
                is_chosen[j] = 1
        fastest += [tmp]
    return fastest
